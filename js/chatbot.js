/* ============================================================
   ...
   A friendly, keyword-based assistant with a female persona
   (Dr. Sana 👩‍⚕️). No API key, no external calls — it runs
   entirely in the browser and answers common health questions.
   Every answer carries a gentle "not a doctor" note and steers
   serious or emergency issues toward real care.

   To teach it new answers: add a block to the RULES array below.
   Each rule = { keywords: [...], reply: "..." }. The first rule
   whose keyword appears in the message wins.
   ============================================================ */

const BOT = '👩‍⚕️';   // the assistant's face, shown by each answer

function el(id) { return document.getElementById(id); }

/* ---- add a message (with an avatar beside it) ---- */
...
const RULES = [
  ...
  { keywords: ['diet', 'eat', 'nutrition', 'healthy food', 'vegetables'],
    reply: "A simple, flexible aim is plenty of plants — vegetables, fruit, beans, whole grains, nuts and seeds — across the week, with regular meals. Variety matters more than any single 'superfood'." },

  { keywords: ['exercise', 'workout', 'active', 'walk', 'steps'],
    reply: "Around 150 minutes of movement a week helps, and it doesn't need to be the gym. Short 'exercise snacks' — a brisk 2–5 minute walk, taking the stairs — add up, and a walk after meals steadies blood sugar." },
...
function handleSend(text) {
  ...
  el('chat-input').value = '';
  addMsg(text, 'user');
  // small, human-feeling pause before the answer
  setTimeout(() => addMsg(findReply(text), 'bot', { disclaimer: true }), 350);
}
